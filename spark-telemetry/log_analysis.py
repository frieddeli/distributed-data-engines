...
# # **Log Analysis Pipeline: Web Server Log Analysis with Apache Spark**
# 
# This lab will demonstrate how easy it is to perform web server log analysis with Apache Spark.
# 
# Server log analysis is an ideal use case for Spark.  It's a very large, common data source and contains a rich set of information.  Spark allows you to store your logs in files on disk cheaply, while still providing a quick and simple way to perform data analysis on them.  This pipeline will show you how to use Apache Spark on real-world text-based production logs and fully harness the power of that data.  Log data comes from many sources, such as web, file, and compute servers, application logs, user-generated content,  and can be used for monitoring servers, improving business and customer intelligence, building recommendation systems, fraud detection, and much more.
# 
# **Pipeline Architecture**
# 
# This pipeline is broken up into sections with bite-sized examples for demonstrating Spark functionality for log processing. For each problem, you should start by thinking about the algorithm that you will use to *efficiently* process the log in a parallel, distributed manner. This means using the various [RDD](https://spark.apache.org/docs/latest/api/python/reference/api/pyspark.RDD.html) operations along with [`lambda` functions](https://docs.python.org/3/tutorial/controlflow.html#lambda-expressions) that are applied at each worker.
# 
...
# stop any existing context first to avoid the "multiple SparkContexts" error
...
# #### **Part 1: Apache Web Server Log file format**
# 
# The log files that we use for this pipeline are in the [Apache Common Log Format (CLF)](https://httpd.apache.org/docs/). The log file entries produced in CLF will look something like this:
...
# 
# For this pipeline, we will use a data set from NASA Kennedy Space Center WWW server in Florida. The full [data set](http://ita.ee.lbl.gov/html/contrib/NASA-HTTP.html) is freely available and contains two month's of all HTTP requests. We are using a subset that only contains several hours worth of requests.
# 
...
# 
# To create the primary RDD that we'll use in the rest of this pipeline, we first load the text file using [`sc.textfile(logFile)`](https://spark.apache.org/docs/latest/api/python/reference/api/pyspark.SparkContext.textFile.html) to convert each line of the file into an element in an RDD.
# 
...
# endpoint when no protocol is present
APACHE_ACCESS_LOG_PATTERN = '^(\S+) (\S+) (\S+) \[([\w:/]+\s[+\-]\d{4})\] "(\S*)\s*([^"]*?)\s*(HTTP/\S+)?" (\d{3}) (\S+)'
...
# filter out all the successful 200 responses, only keep the non-200 ones
not200 = access_logs.filter(lambda log: log.response_code != 200)

# map each bad record to a (endpoint, 1) pair
endpointCountPairTuple = not200.map(lambda log: (log.endpoint, 1))

# sum up the counts per endpoint
endpointSum = endpointCountPairTuple.reduceByKey(lambda a, b: a + b)

# grab top 5 by count descending
...
# remove duplicates to get unique hosts
...
# remove duplicate (hour, host) pairs so each host is counted once per hour
hourGroupedHosts = hourToHostPairTuple.distinct()

# map each unique (hour, host) pair to (hour, 1) then sum per hour
hourHostCount = hourGroupedHosts.map(lambda x: (x[0], 1)).reduceByKey(lambda a, b: a + b)

# sort by hour and cache for reuse
hourlyHosts = hourHostCount.sortByKey().cache()
...
# sum up all requests per hour
groupedByHour = hourAndHostTuple.reduceByKey(lambda a, b: a + b)

# sort by hour so hours are in order
sortedByHour = groupedByHour.sortByKey()

# join with hourlyHosts to get (hour, (total_requests, unique_hosts)), then divide
# need sortByKey again after join since join doesn't guarantee order
...
# **(4c) Exercise: Listing the Top Five 404 Response Code Endpoints**
# 
# Using the RDD containing only log records with a 404 response code that you cached in part (4a), print out a list of the top five endpoints that generate the most 404 errors.
# 
...
# sum up counts per endpoint
badEndpointsSum = badEndpointsCountPairTuple.reduceByKey(lambda a, b: a + b)

# take top 5 sorted by count descending
badEndpointsTop5 = badEndpointsSum.takeOrdered(5, key=lambda x: -x[1])
print('Top Five 404 URLs: %s' % badEndpointsTop5)
...
Test.assertEquals(badEndpointsTop5,
                  [(u'/pub/winvn/readme.txt', 20), (u'/pub/winvn/release.txt', 19),
                   ...
                   (u'/history/apollo/publications/sp-350/sp-350.txt~', 12)],
...
# sum up errors per hour
hourRecordsSum = hourCountPairTuple.reduceByKey(lambda a, b: a + b)

# sort by hour and cache since we reuse it below
