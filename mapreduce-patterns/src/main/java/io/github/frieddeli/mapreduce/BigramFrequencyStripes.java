...
public class BigramFrequencyStripes extends Configured implements Tool {
		...
		public void map(LongWritable key, Text value, Context context)
			...
			if (words.length > 1) {
				...
				for (int i = 1; i < words.length; i++) {
					...
					// add w to the stripe for the current key word
					STRIPE.increment(w);
					context.write(KEY, STRIPE);
					// move to the next word and reset the stripe
					KEY.set(w);
					STRIPE.clear();
				}
			}
	...
	private static class MyReducer extends
		...
		public void reduce(Text key,
			...
			// sum all values in the stripe to get total occurrences of this word
			float total = 0;
			...
			// output (word, next_word) -> relative frequency p(next_word | word)
