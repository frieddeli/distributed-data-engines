		...
		public void map(LongWritable key, Text value, Context context)
			...
			if (words.length > 1) {
				...
				for (int i = 1; i < words.length; i++) {
					...
					// emit (prev, *) so the reducer knows the total count for "prev"
					// "*" sorts before any real word, so it arrives first at the reducer
					BIGRAM.set(prev, "*");
					context.write(BIGRAM, ONE);
					// emit the actual bigram (prev, w)
					BIGRAM.set(prev, w);
					context.write(BIGRAM, ONE);
					prev = w;
				}
	...
	private static class MyReducer extends
		...
		public void reduce(PairOfStrings key, Iterable<IntWritable> values,
			...
			if (key.getRightElement().equals("*")) {
				...
				// save it so we can divide bigram counts by it later
				marginal = sum;
				// output (word, "") -> total count as required by the spec
				OUTPUT_KEY.set(key.getLeftElement(), "");
				VALUE.set(marginal);
				context.write(OUTPUT_KEY, VALUE);
			} else {
				// divide bigram count by marginal to get p(w | prev)
