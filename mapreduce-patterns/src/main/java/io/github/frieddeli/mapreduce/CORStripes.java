package io.github.frieddeli.mapreduce;
...
public class CORStripes extends Configured implements Tool {
	...
	public static class CORStripesMapper2 extends Mapper<LongWritable, Text, Text, MapWritable> {
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			// for each word a, build a stripe containing all words b > a in this line
			// this ensures each pair (a, b) is only counted once
			for (int i = 0; i < sortedList.size(); i++) {
				MapWritable stripe = new MapWritable();
				for (int j = i + 1; j < sortedList.size(); j++) {
					stripe.put(new Text(sortedList.get(j)), new IntWritable(1));
				}
				if (!stripe.isEmpty()) {
					context.write(new Text(sortedList.get(i)), stripe);
				}
			}
		}
	}
		...
		protected void reduce(Text key, Iterable<MapWritable> values, Context context) throws IOException, InterruptedException {
			...
			for (MapWritable stripe : values) {
				for (Map.Entry<Writable, Writable> entry : stripe.entrySet()) {
					...
					IntWritable existing = (IntWritable) sum.get(k);
					if (existing == null) {
						sum.put(new Text(k.toString()), new IntWritable(v));
					} else {
						existing.set(existing.get() + v);
					}
				}
			}
	...
	public static class CORStripesReducer2 extends Reducer<Text, MapWritable, PairOfStrings, DoubleWritable> {
		...
		protected void reduce(Text key, Iterable<MapWritable> values, Context context) throws IOException, InterruptedException {
			...
			String aWord = key.toString();
			Integer freqA = word_total_map.get(aWord);
			// skip if word wasn't found in the first pass result
			if (freqA == null) return;
			// iterate over all co-occurring words b and compute cor(a, b)
			for (Map.Entry<Writable, Writable> entry : sum.entrySet()) {
				String bWord = entry.getKey().toString();
				int freqAB = ((IntWritable) entry.getValue()).get();
				Integer freqB = word_total_map.get(bWord);
				if (freqB == null) continue;
				// cor(a, b) = freq(a,b) / (freq(a) * freq(b))
				double cor = (double) freqAB / ((double) freqA * freqB);
				context.write(new PairOfStrings(aWord, bWord), new DoubleWritable(cor));
			}
		}
	}
