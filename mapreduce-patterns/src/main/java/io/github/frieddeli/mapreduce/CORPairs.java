...
public class CORPairs extends Configured implements Tool {
	...
	public static class CORPairsMapper2 extends Mapper<LongWritable, Text, PairOfStrings, IntWritable> {
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			// sort so we always emit (smaller, larger) and avoid double counting
			List<String> sorted_words = new ArrayList<String>(unique_words);
			Collections.sort(sorted_words);
			// emit 1 for every unique pair (a, b) in this line
			for (int i = 0; i < sorted_words.size(); i++) {
				for (int j = i + 1; j < sorted_words.size(); j++) {
					context.write(new PairOfStrings(sorted_words.get(i), sorted_words.get(j)), new IntWritable(1));
				}
			}
		}
	}
	...
	public static class CORPairsReducer2 extends Reducer<PairOfStrings, IntWritable, PairOfStrings, DoubleWritable> {
		...
		protected void reduce(PairOfStrings key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
			...
			// skip if either word wasn't seen in the first pass
			if (freqA == null || freqB == null) return;
			// cor(a, b) = freq(a,b) / (freq(a) * freq(b))
			double cor = (double) sum / ((double) freqA * freqB);
